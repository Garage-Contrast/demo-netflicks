...
'''
        ...
        container('openshift') {
          sh "cd k8s && DEPLOYMENT=${env.DEPLOYMENT} NAMESPACE=${env.PROJECT} GIT_COMMIT=${env.GIT_COMMIT} envsubst '\$DEPLOYMENT,\$NAMESPACE,\$GIT_COMMIT' < \"kustomization-template.yaml\" > \"kustomization.yaml\""
        }
      }
    }
    stage('deploy') {
